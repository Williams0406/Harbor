...
const STATUS_OPTS    = [{ value:'pending',label:'Pendiente'},{ value:'in_process',label:'En proceso'},{ value:'cancelled',label:'Cancelado'}]
...
export default function SalesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [res, cRes] = await Promise.all([salesApi.getSales(params), contactsApi.getContacts()])
  ...
  async function handleDelete(id) {
    ...
    await salesApi.deleteSale(id); load()
  }

  async function handleSave() {
    if (!form.voucher_type || !form.voucher_number) { setError('Tipo y número son requeridos'); return }
    setSaving(true)
    try {
      const payload = { ...form, contact: form.contact || null }
      editing ? await salesApi.updateSale(editing.id, payload) : await salesApi.createSale(payload)
      setModal(false); load()
  ...
  return (
    <PageShell
      ...
      actions={<Button onClick={openCreate}>+ Nueva venta</Button>}
    >
      ...
      <Table columns={columns} data={data} loading={loading} emptyMessage="No hay ventas registradas" />

      <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'Editar venta' : 'Nueva venta'}>
        <div className={styles.form}>
          <div className={styles.row}>
            <Input label="Tipo de comprobante *" value={form.voucher_type} onChange={set('voucher_type')} placeholder="Factura, Boleta..." />
          ...
          <div className={styles.modalActions}>
            ...
            <Button loading={saving} onClick={handleSave}>{editing ? 'Guardar cambios' : 'Crear venta'}</Button>
          </div>
        </div>
      </Modal>
    </PageShell>
